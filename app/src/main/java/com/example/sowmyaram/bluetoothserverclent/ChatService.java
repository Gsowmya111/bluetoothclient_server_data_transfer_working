...
//1st we need to create this accept thread to connect it helps to wait for connection
    // runs while listening for incoming connections
    private class AcceptThread extends Thread {
        private final BluetoothServerSocket serverSocket;
        private String socketType;
//constructor of accept thread
        public AcceptThread(boolean secure) {
            ...
            try {
                if (secure) {
                    ...
                    //listening server socket with rfcomm with appname and uuid
                    tmp = bluetoothAdapter.listenUsingRfcommWithServiceRecord(
                            NAME_SECURE, MY_UUID_SECURE);
                    Log.d(TAG,"Accept Thread : setting my server using "+MY_UUID_SECURE);

                } else {
                    tmp = bluetoothAdapter
                            .listenUsingInsecureRfcommWithServiceRecord(
                                    NAME_INSECURE, MY_UUID_INSECURE);
                    Log.d(TAG,"Accept Thread : setting my server using "+MY_UUID_INSECURE);
                }
            } catch (IOException e) {
            }
            serverSocket = tmp;
        ...
        public void run() {
            ...
            while (state != STATE_CONNECTED) {
                ...
                //this is blocking code where exception occur it will
                //return only connection is succesful otherwise exception occur

                try {

                    //code sit here untill connection hapens
